...
function resolveEmbedUrl(rawUrl) {
  try {
    ...
    // ── Bilibili (DÜZELTME: bilibili.com + BV/av ID desteği) ────────────────
    // bilibili.tv iframe desteklemiyor; bilibili.com/video/BVxxx destekleniyor
    if (host === 'bilibili.com') {
      ...
      if (match) {
        ...
        return {
          type: 'embed',
          url:  `https://player.bilibili.com/player.html?${param}&autoplay=1&high_quality=1`,
        };
      }
    }
...
app.get('/proxy', async (req, res) => {
  ...
  try {
    ...
    // DÜZELTME: Binary içerik → önce buffer'a al, sonra gönder
    // (pipe() header'dan sonra çakışma yapabiliyordu)
    ...
    // iframe içindeki video olaylarını (play/pause/seek) parent'a iletir
    // ve parent'tan gelen komutları (play/pause/seek) uygular
    const syncScript = `
<style>
...
(function(){
  ...
  function applyCmd(action, time){
    // Önce bu frame'deki video elementlerini dene
    ...
    // İç iframe'lere de ilet (iç içe embed durumu)
...
app.post('/api/room/create', (req, res) => {
  ...
  rooms[roomId] = {
    ...
    users:       {},
    hostId:      null,
    currentTime: 0,       // video senkronizasyon için
    lastAction:  null,    // 'play' | 'pause'
  };
  res.json({ roomId });
});
  ...
  ws.on('message', (raw) => {
    ...
    switch (msg.type) {
      ...
      case 'join': {
        ...
        // DÜZELTME: currentTime ve lastAction da gönderiliyordu ama eksikti
      ...
      case 'set_url': {
        ...
        room.url = msg.url;
        // Yeni URL yüklenince sync sıfırla
        room.currentTime = 0;
      ...
      case 'comment': {
        if (!currentRoom) return;
        const room = rooms[currentRoom];
        if (!msg.text || !msg.text.trim()) return; // boş yorum engelle
        const comment = {
          id:       uuidv4().slice(0, 8),
          userId,
          nickname,
        ...
        // DÜZELTME: shift() yerine splice — daha güvenli
        if (room.comments.length > 200) room.comments.splice(0, 1);
        broadcast(currentRoom, { type: 'new_comment', comment });
        break;
      }

      // DÜZELTME: video_sync mesajı eksikti, eklendi
      // Sadece host senkronizasyon gönderebilir
      case 'video_sync': {
        if (!currentRoom) return;
        const room = rooms[currentRoom];
        if (room.hostId !== userId) return;
